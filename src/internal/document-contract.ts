...
type TextDocumentContractErrorCode =
  ...
  | "duplicate-block-id"
  | "duplicate-span-id"
  | "duplicate-animation-unit-id"
  | "invalid-range"
  | "dangling-parent-block"
  | "dangling-child-block"
  | "dangling-child-span"
  | "dangling-span-block"
  | "dangling-animation-span"
  | "invalid-child-block-parent"
  | "invalid-child-span-block"
  | "noncontiguous-animation-ordinal"
  | "animation-text-mismatch"
  | "static-animation-units";
...
export class TextDocumentContractError extends Error {
  ...
  constructor(
    code: TextDocumentContractErrorCode,
    message: string,
...
export function createEmptyTextDocument({
  sourceKind = "plain",
  text = "",
}: {
...
export function assertValidTextDocument(document: TextDocument): TextDocument {
  if (document.version !== TEXT_DOCUMENT_VERSION) {
    throw new TextDocumentContractError(
      "unsupported-version",
      `Unsupported text document version ${document.version}`,
      {
  ...
  if (document.source.kind !== "plain" && document.source.kind !== "markdown") {
    throw new TextDocumentContractError(
      "invalid-source-kind",
      `Unsupported text source kind ${document.source.kind}`,
      {
  ...
  if (
    ...
    throw new TextDocumentContractError(
      "invalid-animation-mode",
      `Unsupported animation mode ${document.animationPlan.mode}`,
      {
  ...
  const spansById = new Map<string, TextDocument["spans"][number]>();
  const animationUnitIds = new Set<string>();
  ...
  for (const block of document.blocks) {
    if (block.parentBlockId && !blocksById.has(block.parentBlockId)) {
      throw new TextDocumentContractError(
        "dangling-parent-block",
        `Block ${block.id} references missing parent ${block.parentBlockId}`,
        {
          blockId: block.id,
    ...
    for (const childSpanId of block.childSpanIds) {
      ...
      if (!childSpan) {
        throw new TextDocumentContractError(
          "dangling-child-span",
          `Block ${block.id} references missing child span ${childSpanId}`,
          {
            blockId: block.id,
            childSpanId,
          },
      ...
      if (childSpan.blockId !== block.id) {
        throw new TextDocumentContractError(
          "invalid-child-span-block",
          `Span ${childSpanId} belongs to ${childSpan.blockId}, not ${block.id}`,
          {
            blockId: block.id,
            childSpanId,
    ...
    for (const childBlockId of block.childBlockIds ?? []) {
      ...
      if (childBlock.parentBlockId !== block.id) {
        throw new TextDocumentContractError(
          "invalid-child-block-parent",
          `Child block ${childBlockId} belongs to ${childBlock.parentBlockId ?? "none"}, not ${block.id}`,
          {
            blockId: block.id,
            childBlockId,
  ...
  for (const span of document.spans) {
    ...
    if (!ownerBlock) {
      throw new TextDocumentContractError(
        "dangling-span-block",
        `Span ${span.id} references missing block ${span.blockId}`,
        {
          spanId: span.id,
  ...
  const sortedUnits = [...document.animationPlan.units].sort(
  ...
  if (document.animationPlan.mode === "static") {
    if (sortedUnits.length > 0) {
      throw new TextDocumentContractError(
        "static-animation-units",
        "Static animation plans must not contain units",
        {
  ...
  for (const [expectedOrdinal, unit] of sortedUnits.entries()) {
    if (animationUnitIds.has(unit.id)) {
      throw new TextDocumentContractError(
        "duplicate-animation-unit-id",
        `Duplicate animation unit id ${unit.id}`,
        {
          unitId: unit.id,
        },
      );
    ...
    if (unit.ordinal !== expectedOrdinal) {
      throw new TextDocumentContractError(
        "noncontiguous-animation-ordinal",
        `Animation unit ordinal ${unit.ordinal} does not match expected ${expectedOrdinal}`,
        {
          expectedOrdinal,
    ...
    if (!parentSpan) {
      throw new TextDocumentContractError(
        "dangling-animation-span",
        `Animation unit ${unit.id} references missing span ${unit.spanId}`,
        {
          unitId: unit.id,
          spanId: unit.spanId,
        },
  ...
  if (expectedVisibleText !== actualVisibleText) {
    throw new TextDocumentContractError(
      "animation-text-mismatch",
      "Animation units do not reconstruct the visible text",
      {
        expectedVisibleText,
        actualVisibleText,
      },
...
export function serializeTextDocument(
  ...
  const throwOnValidationError =
    options?.throwOnValidationError ?? shouldThrowOnValidationError();
...
function shouldThrowOnValidationError(): boolean {
...
function assertValidRange(
  ...
  if (!Number.isInteger(start) || !Number.isInteger(end)) {
    throw new TextDocumentContractError(
      "invalid-range",
      `Range for ${label} must contain integer offsets`,
      {
        label,
        range,
      },
  ...
  if (start < 0 || end < start || end > sourceLength) {
    throw new TextDocumentContractError(
      "invalid-range",
      `Range for ${label} is out of bounds`,
      {
        label,
        range,
        sourceLength,
...
function assertRangeWithin(
  ...
  if (childRange[0] < parentRange[0] || childRange[1] > parentRange[1]) {
    throw new TextDocumentContractError(
      "invalid-range",
      `Range for ${label} must stay within its parent range`,
      {
        label,
        childRange,
        parentRange,
...
function normalizeContractError(error: unknown): TextDocumentContractError {
  ...
  return new TextDocumentContractError(
    "invalid-range",
    "Unknown text document contract failure",
  );
}
