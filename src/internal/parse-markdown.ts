...
import type { TextBlock, TextRange, TextSpan, TextSpanKind } from "./types.js";
...
type MarkdownBlockInput =
  | { type: "heading"; level: number; text: string; range: TextRange }
  | { type: "paragraph"; text: string; range: TextRange }
  | { type: "blockquote"; text: string; range: TextRange }
  | {
      type: "bulletList";
      items: { text: string; range: TextRange }[];
      range: TextRange;
    }
  | {
      type: "orderedList";
  ...
  | { type: "code"; language: string | null; text: string; range: TextRange }
  | { type: "hr"; range: TextRange };
  ...
  for (const [blockIndex, block] of parseMarkdownBlocks(source).entries()) {
    ...
    if (block.type === "bulletList" || block.type === "orderedList") {
      ...
      for (const [itemIndex, item] of block.items.entries()) {
        const itemBlockId = createStableId(
          "block",
          listBlockId,
          "item",
          itemIndex,
    ...
    blocks.push({
      ...
      childSpanIds,
      metadata:
        block.type === "code" ? { fenceLanguage: block.language } : undefined,
    });
  }
...
function appendInlineSpans({
  ...
  for (const [index, token] of tokens.entries()) {
    ...
    spans.push({
      ...
      animationGroupId: createStableId(
        "anim-group",
        blockId,
        token.range[0],
        token.range[1],
...
function parseMarkdownBlocks(source: string): MarkdownBlockInput[] {
  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (/^\s*[-*+]\s+/.test(line)) {
      const start = lineStarts[index];
      const items: { text: string; range: TextRange }[] = [];
      while (index < lines.length && /^\s*[-*+]\s+/.test(lines[index])) {
        items.push({
...
function lineEnd(lineStarts: number[], lines: string[], index: number): number {
  if (index < 0) {
    return 0;
  }
